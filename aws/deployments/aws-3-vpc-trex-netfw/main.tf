...
# Hub VPC
# ==============================================================================
# One VPC for everything - AWS Network Firewall's natural shape here.
...
# CIDR and TRex's spoofed range at the firewall's VPC endpoint; the firewall
# subnet then routes both onward to the real trex1/trex2 ENIs.
...
module "hub_routing" {
  ...
  route_tables = {
    ...
    "app-01" = {
      ...
      routes = [
        {
          # trex1 (app-01) -> AWS Network Firewall -> ... -> trex2
          destination_cidr_block = module.hub.subnet_cidr_blocks["app-02"]
          target_type            = "vpc_endpoint"
          target_id              = module.firewall.endpoint_ids_by_subnet[module.hub.subnet_ids["firewall"]]
        },
        {
          # trex1's actual outbound packets are addressed to the trex
          # server range, not app-02's real CIDR - same firewall endpoint,
    ...
    "app-02" = {
      ...
      routes = [
        {
          # trex2 (app-02) -> AWS Network Firewall -> ... -> trex1
...
# pass-through; AWS Network Firewall's own rule group is the mechanism under test.
...
module "vm_app_01" {
  ...
  network_interfaces = {
    # Own subnet (mgmt), matching aws-1-vpc/aws-2-vpc-trex.
    mgmt = {
...
# DNS
# ==============================================================================
# A record for the instance's mgmt Elastic IP. None for the firewall itself
# - it's fully managed, no instance to name.
