...
variable "spoke_1_subnets" {
  ...
  default = {
    ...
    # directly to trex2's NIC - no NVA here.
    "snet-app-01" = {
      address_prefixes = ["10.10.1.0/24"]
      routes = [
        {
          name                   = "trex-servers-via-trex2"
          address_prefix         = "48.0.0.0/8"
          next_hop_type          = "VirtualAppliance"
          next_hop_in_ip_address = "10.10.2.10" # trex2 / App2 NIC (cidrhost("10.10.2.0/24", 10))
        }
      ]
    }
    # trex2 (server, 48.0.0.0/8) - mirrors snet-app-01's route, return
    # direction, back to trex1's NIC (App1).
    "snet-app-02" = {
      address_prefixes = ["10.10.2.0/24"]
      routes = [
        {
          name                   = "trex-clients-via-trex1"
          address_prefix         = "16.0.0.0/8"
          next_hop_type          = "VirtualAppliance"
          next_hop_in_ip_address = "10.10.1.10" # trex1 / App1 NIC (cidrhost("10.10.1.0/24", 10))
        }
      ]
    }
  }
