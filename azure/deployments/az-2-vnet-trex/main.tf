...
# Holds the hub VNet and its route tables.
...
# Routes for spoke 1's dedicated tables, sourced from each subnet's `routes`
# entry (local.spoke_1_subnet_routes).
...
# Hub VNet: AzureFirewallSubnet + AzureBastionSubnet.
module "hub" {
  source = "../../modules/vnet"
