...
# Holds the hub VNet, FortiGate, and their route tables.
...
# Dedicated table for the "firewall" subnet (FortiGate's port2 NIC, fgt.tf),
# separate from hub_rt so these two routes don't also apply to "mgmt".
...
# Forward TRex traffic from FortiGate's port2 on to the other trex NIC -
# works via enable_ip_forwarding = true on trex1/trex2.
...
# to FortiGate's port2 IP; an explicit value is left untouched.
resource "azurerm_route" "spoke_1_routes" {
  ...
  next_hop_in_ip_address = (
    ...
    : each.value.next_hop_type == "VirtualAppliance" ? local.fgt_port2_ip : null
  )
}
...
# Hub VNet: "firewall" (FortiGate port2) + "mgmt" subnets.
module "hub" {
  source = "../../modules/vnet"
...
# A record for FortiGate's port1 public IP.
resource "azapi_resource" "fortigate_dns_record" {
  type      = "Microsoft.Network/dnszones/A@2018-05-01"
  name      = module.fortigate.vm_name
  parent_id = var.dns_zone_id
  ...
  body = {
    properties = {
      ...
      ARecords = [
        {
          ipv4Address = module.fortigate.port1_public_ip
        }
      ]
    }
  }
