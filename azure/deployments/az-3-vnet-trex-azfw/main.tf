...
# Holds the hub VNet, Azure Firewall, and their route tables.
...
# Default route table for hub subnets other than AzureFirewallSubnet.
...
# Separate from hub_rt - AzureFirewallSubnet requires its own route table
# with a mandatory default route before association.
resource "azurerm_route_table" "hub_azfw_rt" {
  name                = "udr-${var.environment}-hub-azfw"
  location            = azurerm_resource_group.hub.location
  resource_group_name = azurerm_resource_group.hub.name
  tags                = local.tags
}

# The mandatory default route AzureFirewallSubnet's route table requires.
resource "azurerm_route" "hub_azfw_default_to_internet" {
  name                = "default-to-internet"
  resource_group_name = azurerm_resource_group.hub.name
  route_table_name    = azurerm_route_table.hub_azfw_rt.name
  address_prefix      = "0.0.0.0/0"
  next_hop_type       = "Internet"
}

# Forward TRex traffic onward to the other trex NIC once firewall policy
# clears it - works via enable_ip_forwarding = true on trex1/trex2.
resource "azurerm_route" "hub_azfw_to_trex1" {
...
# Return direction of hub_azfw_to_trex1, toward trex2.
resource "azurerm_route" "hub_azfw_to_trex2" {
...
resource "azurerm_route" "hub_azfw_to_trex_data_port0" {
...
# Return direction of hub_azfw_to_trex_data_port0, toward trex2.
resource "azurerm_route" "hub_azfw_to_trex_data_port1" {
...
# Routes from var.spoke_1_subnets (local.spoke_1_subnet_routes). null
# next_hop_in_ip_address falls back to module.firewall.private_ip_address
# (only known after apply); an explicit value is left untouched.
resource "azurerm_route" "spoke_1_routes" {
  ...
  next_hop_in_ip_address = (
    ...
    : each.value.next_hop_type == "VirtualAppliance" ? module.firewall.private_ip_address : null
  )
}
...
module "hub" {
  ...
  route_table_associations = merge(
    ...
    contains(keys(var.hub_subnets), "AzureFirewallSubnet") ? {
      "AzureFirewallSubnet" = azurerm_route_table.hub_azfw_rt.id
    } : {}
  )
  # Default route must exist before the association - Azure rejects it
  # otherwise.
...
# DNS
# ==============================================================================
# A record for the VM's mgmt public IP, named after the VM, in the
