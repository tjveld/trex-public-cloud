...
#
# Output: {"meta": {cloud, vm_name, region, vm_sku, frame_size, mult, pg_ids, latency_pps, warmup,
#          duration, timestamp}, "stats": {...}}
...
from trex_stl_lib.api import STLClient, STLStream, STLPktBuilder, STLTXCont, STLFlowLatencyStats
...
# RFC 2544 standard frame sizes (bytes). IP_ADDRESSES swaps src/dst by direction so --bidir produces
# real bidirectional traffic instead of mirroring one direction onto both ports. A single fixed
# address each side (not a range) - bulk traffic uses a single fixed UDP port (UDP_PORT).
# The latency stream uses its own (LATENCY_UDP_PORT), so the two are distinguishable in firewall logs.
RFC2544_FRAME_SIZES = (64, 128, 256, 512, 1024, 1280, 1518)
IP_ADDRESSES = {
    "src": "16.0.0.1",
    "dst": "48.0.0.1",
}
# Fixed UDP source and destination port for the bulk stream (scapy's UDP() default is sport=dport=53).
UDP_PORT = 12
...
def _build_packet(src, dst, frame_size, udp_port=UDP_PORT, udp_chksum=None):
    """Build a fresh STLPktBuilder for one fixed src/dst IP pair (no per-packet address variation).
    ...
    the latency stream (see build_streams)."""
    base_pkt = Ether() / IP(src=src, dst=dst) / UDP(sport=udp_port, dport=udp_port, chksum=udp_chksum)
    pad = max(0, (frame_size - FCS_BYTES) - len(base_pkt)) * "x"
    return STLPktBuilder(pkt=base_pkt / pad)
...
def build_streams(direction, frame_size, latency_pps):
    ...
    payload with its own latency header after scapy has already computed the UDP checksum over the
    original bytes, so the checksum that ships is wrong for what's actually on the wire - a stateful
    firewall (e.g. Azure Firewall) validates it, sees a malformed packet, and drops it silently (no
    rule-match log entry, since a malformed-packet drop isn't a rule decision)."""
    src, dst = (IP_ADDRESSES["dst"], IP_ADDRESSES["src"]) if direction else (IP_ADDRESSES["src"], IP_ADDRESSES["dst"])
